...
export function Button({ className, variant = 'primary', loading, children, ...props }) { return <button className={clsx('button', `button-${variant}`, className)} disabled={loading || props.disabled} {...props}>{loading ? 'Working…' : children}</button> }
...
export function FormField({ label, children, htmlFor }) { return <label className="form-field" htmlFor={htmlFor}><span>{label}</span>{children}</label> }
