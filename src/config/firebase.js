...
const coreKeys = ['VITE_FIREBASE_API_KEY', 'VITE_FIREBASE_AUTH_DOMAIN', 'VITE_FIREBASE_PROJECT_ID', 'VITE_FIREBASE_STORAGE_BUCKET', 'VITE_FIREBASE_MESSAGING_SENDER_ID', 'VITE_FIREBASE_APP_ID']
const config = { apiKey: import.meta.env.VITE_FIREBASE_API_KEY, authDomain: import.meta.env.VITE_FIREBASE_AUTH_DOMAIN, projectId: import.meta.env.VITE_FIREBASE_PROJECT_ID, storageBucket: import.meta.env.VITE_FIREBASE_STORAGE_BUCKET, messagingSenderId: import.meta.env.VITE_FIREBASE_MESSAGING_SENDER_ID, appId: import.meta.env.VITE_FIREBASE_APP_ID, databaseURL: import.meta.env.VITE_FIREBASE_DATABASE_URL }
export const isFirebaseConfigured = coreKeys.every((key) => Boolean(import.meta.env[key]))
export const isRealtimeDatabaseConfigured = Boolean(config.databaseURL)
...
let app; let auth; let db; let rtdb
if (isFirebaseConfigured) {
  app = getApps()[0] || initializeApp(config)
  auth = getAuth(app)
  if (isRealtimeDatabaseConfigured) rtdb = getDatabase(app)
...
export { app, auth, db, rtdb, cacheEnabled }
